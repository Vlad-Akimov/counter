...
class MetalSheetExtractor:
    ...
    def __init__(self, density_threshold: float = 0.3, debug: bool = False):
        """
        Initialize the Metal Sheet Extractor.
        
        Args:
            density_threshold: Minimum edge density for sheet region (default: 0.3)
    ...
    def extract_sheet_region(self, image_path: str, padding: int = 10) -> Tuple[np.ndarray, SheetRegion]:
        ...
        print(f"[4/5] Computing edge density map (threshold: {self.density_threshold})...")
        ...
        high_density_mask = density_map > self.density_threshold
        ...
        # Step 4: Refine boundaries (optional but keeps the density-based approach)
        ...
        if self.debug:
            self._visualize_debug(img_rgb, density_map, high_density_mask, 
                                 sheet_region, edge_binary, gradient_magnitude)
        ...
        print(f"Edge density score: {density_score:.3f} (threshold: {self.density_threshold})")
    ...
    def _visualize_debug(self, original_img: np.ndarray, density_map: np.ndarray,
        ...
        plt.title(f'Density > {self.density_threshold}')
        plt.axis('off')
...
def main(pat):
    ...
    try:
        ...
        extractor = MetalSheetExtractor(
            density_threshold=DENSITY_THRESHOLD,
            debug=DEBUG_MODE
        )
        
        # Extract single image
        extracted_img, region_info = extractor.extract_sheet_region(
